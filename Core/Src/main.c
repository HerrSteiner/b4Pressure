...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	float filterInput = (sample * osc1.outputFilter + sample2 * osc2.outputFilter) * 0.25f;
	...
	if (filterModus == GPIO_PIN_RESET){
		float cutoffAndMod = cutoff + (eg1.destinations[3]*env1Value + eg2.destinations[3]*env2Value) * 0.25f + (pad[1].mode2 * pad[1].pressure + pad[2].mode2 * pad[2].pressure) * 0.5f;
		...
		// Moogish filter
		float f = (cutoff + (eg1.destinations[3]*env1Value*0.2f) + (eg2.destinations[3]*env2Value*0.2f) + pad[1].mode2 * pad[1].pressure + pad[2].mode2 * pad[2].pressure);
